...
trait InstallHelperTrait
{
	 ...
	 * @return array
	 */
	public function entityColumnsToJson($entityName, array $columns, $position, array $stepData, $singlePass = false)
	{
		...
		return $this->tableColumnsToJson($structure->table, $primaryKeys, $columns, $position, $stepData, $singlePass);
	}
	...
	public function tableColumnsToJson(
		$tableName, array $primaryKeys, array $serializedFields, $position, array $stepData, $singlePass = false, $regex = '^([abCdioOsS]:|N;$)'
	)
	{
		if (!strlen($regex))
		{
		...
		foreach ($serializedFields AS $column)
		{
			$whereSql[] = "({$column} <> '' AND {$column} REGEXP '{$regex}')";
		}
