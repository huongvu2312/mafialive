...
class EmojiFormatter
{
	...
	public function formatShortnameToEmoji($string, $native = true)
	{
		...
		$string = preg_replace_callback('/' . $client->ignoredRegexp . '|(' . $client->shortcodeRegexp . ')/Si', function($match) use($client, $native)
		{
			if (!is_array($match) || !isset($match[1]) || empty($match[1]))
			{
				return $match[0];
	...
	protected function setTypeSpecificDefaults($config)
	{
		...
		{
			$config['path'] = $useCdn ? 'https://cdn.jsdelivr.net/emojione/assets/4.0/png/64/' : $config['path'];
