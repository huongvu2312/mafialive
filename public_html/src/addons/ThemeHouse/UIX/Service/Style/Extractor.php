...
class Extractor extends \XF\Service\AbstractService
{
    public function extract($filePath, $directory)
    {
        ...
        if (!is_dir($directory)) {
            try {
                \XF\Util\File::createDirectory($directory);
            } catch (\Exception $e) {
        ...
        for ($i = 0; $i < $zip->numFiles; $i++) {
            ...
            if (strtolower($fileParts[0]) === 'upload' || strpos($fileParts[0], '.xml') !== false) {
                ...
                \XF\Util\File::makeWritableByFtpUser($realFile);
            }

            if (strtolower($fileParts[0]) === 'child_xmls' && !empty($fileParts[1]) && strpos($fileParts[1], '.xml') !== false) {
                $zip->extractTo($directory, array($zip->getNameIndex($i)));
                $realFile = $directory . DIRECTORY_SEPARATOR . $zip->getNameIndex($i);
                \XF\Util\File::makeWritableByFtpUser($realFile);
