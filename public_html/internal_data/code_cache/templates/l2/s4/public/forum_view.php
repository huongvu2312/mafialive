<?php
// FROM HASH: 37ff1b64bb796e1969b7fd8faa48926a
...
<div class="block" data-xf-init="' . ($__vars['canInlineMod'] ? 'inline-mod' : '') . '" data-type="thread" data-href="' . $__templater->fn('link', array('inline-mod', ), true) . '">
	...
	$__finalCompiled .= '
	';
	$__vars['sortOrders'] = array('last_post_date' => 'Bài viết cuối', 'post_date' => 'Tin nhắn đầu tiên', 'title' => 'Tiêu đề', 'reply_count' => 'Trả lời', 'view_count' => 'Xem', 'first_post_reaction_score' => 'First message reaction score', );
	$__finalCompiled .= '
	...
	if ($__vars['filters']['order'] AND $__vars['sortOrders'][$__vars['filters']['order']]) {
		...
		if ($__vars['filters']['direction'] == 'asc') {
			$__compilerTemp4 .= 'Dưới lên';
		} else {
			$__compilerTemp4 .= 'Trên xuống';
		}
	...
	if (!$__templater->test($__vars['stickyThreads'], 'empty', array()) OR !$__templater->test($__vars['threads'], 'empty', array())) {
		...
		if (!$__templater->test($__vars['stickyThreads'], 'empty', array())) {
			$__finalCompiled .= '
						';
			if ($__templater->fn('property', array('uix_separateThreads', ), false) AND (!$__templater->test($__vars['stickyThreads'], 'empty', array()) AND !$__templater->test($__vars['threads'], 'empty', array()))) {
				$__finalCompiled .= '
							<h3 class="block-minorHeader uix_threadListSeparator">
								' . 'Sticky threads' . '
								';
				if ($__templater->fn('property', array('uix_collapsibleStickyThreads', ), false)) {
					$__finalCompiled .= '
									<a href="#" class="uix_threadCollapseTrigger">' . $__templater->fontAwesome('fa-chevron-down', array(
