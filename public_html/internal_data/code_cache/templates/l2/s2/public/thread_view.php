<?php
// FROM HASH: fe3f68c92a760eb043415d083cf62336
...
';
	...
	if ($__vars['xf']['options']['enableTagging'] AND ($__templater->method($__vars['thread'], 'canEditTags', array()) OR $__vars['thread']['tags'])) {
		$__compilerTemp2 .= '
			...
			'editLink' => ($__templater->method($__vars['thread'], 'canEditTags', array()) ? $__templater->fn('link', array('threads/tags', $__vars['thread'], ), false) : ''),
		), $__vars) . '
			</li>
		';
	}
...
<div class="block block--messages" data-xf-init="' . ($__vars['canInlineMod'] ? 'inline-mod' : '') . '" data-type="post" data-href="' . $__templater->fn('link', array('inline-mod', ), true) . '">
	...
	if ($__templater->method($__vars['thread'], 'canCreatePoll', array())) {
		$__compilerTemp8 .= '
											<a href="' . $__templater->fn('link', array('threads/poll/create', $__vars['thread'], ), true) . '" data-xf-click="overlay" class="menu-linkRow">' . 'Tạo khảo sát' . '</a>
										';
	}
	$__compilerTemp8 .= '
										';
	...
	if ($__templater->method($__vars['thread'], 'canReplyBan', array())) {
		$__compilerTemp8 .= '
											<a href="' . $__templater->fn('link', array('threads/reply-bans', $__vars['thread'], ), true) . '" data-xf-click="overlay" class="menu-linkRow">' . 'Quản lý phản hồi bị cấm' . '</a>
										';
	}
	$__compilerTemp8 .= '
										';
	if ($__templater->method($__vars['thread'], 'canViewModeratorLogs', array())) {
		$__compilerTemp8 .= '
											<a href="' . $__templater->fn('link', array('threads/moderator-actions', $__vars['thread'], ), true) . '" data-xf-click="overlay" class="menu-linkRow">' . 'Hành vi quản trị' . '</a>
										';
	}
	$__compilerTemp8 .= '
										' . '
