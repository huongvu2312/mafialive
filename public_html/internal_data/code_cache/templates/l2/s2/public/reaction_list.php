		...
		foreach ($__vars['reactions'] AS $__vars['reaction']) {
			...
			$__vars['extraData'] = $__templater->preEscaped('
				...
				'class' => 'reaction--right',
					...
					' . $__templater->fn('date_dynamic', array($__vars['reaction']['reaction_date'], array(
			))) . '
				');
			$__finalCompiled .= '
	...
	if ($__vars['hasNext']) {
		$__finalCompiled .= '
			<li class="block-footer">
				...
				', array(
			'href' => $__templater->fn('link', array($__vars['link'], $__vars['content'], $__vars['linkParams'] + array('reaction_id' => $__vars['reactionId'], 'list_only' => 1, 'page' => $__vars['page'] + 1, ), ), false),
		...
		if ($__templater->isTraversable($__vars['tabSummary'])) {
			foreach ($__vars['tabSummary'] AS $__vars['reactionId'] => $__vars['total']) {
				...
				if ($__vars['reactionId'] == $__vars['activeReactionId']) {
					...
					$__finalCompiled .= '
						<li data-href="' . $__templater->fn('link', array($__vars['link'], $__vars['content'], $__vars['linkParams'] + array('reaction_id' => $__vars['reactionId'], 'list_only' => 1, ), ), true) . '" class="' . (($__vars['reactionId'] == $__vars['activeReactionId']) ? 'is-active' : '') . '" role="tabpanel" id="reaction-' . $__templater->escape($__vars['reactionId']) . '">
