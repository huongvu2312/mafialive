<?php
// FROM HASH: de1b3f6d640ce72b8ae9503bf34cf02a
return array('macros' => array(), 'code' => function($__templater, array $__vars)
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped($__templater->fn('prefix', array('resource', $__vars['resource'], 'escaped', ), true) . $__templater->escape($__vars['resource']['title']));
	$__finalCompiled .= '

';
	$__vars['descSnippet'] = $__templater->fn('snippet', array($__vars['description']['message'], 250, array('stripBbCode' => true, ), ), false);
	$__finalCompiled .= '

' . $__templater->callMacro('metadata_macros', 'metadata', array(
		'description' => $__vars['descSnippet'],
		'shareUrl' => $__templater->fn('link', array('canonical:resources', $__vars['resource'], ), false),
		'canonicalUrl' => $__templater->fn('link', array('canonical:resources', $__vars['resource'], ), false),
	), $__vars) . '
...
';
	...
	if ($__templater->method($__vars['resource'], 'hasViewableDiscussion', array())) {
		$__compilerTemp4 .= '
			"discussionUrl": "' . $__templater->filter($__templater->fn('link', array('canonical:threads', $__vars['resource']['Discussion'], ), false), array(array('escape', array('json', )),), true) . '",
		';
	}
	...
	<script type="application/ld+json">
	{
		...
		"@type": "CreativeWork",
		"@id": "' . $__templater->filter($__templater->fn('link', array('canonical:resources', $__vars['resource'], ), false), array(array('escape', array('json', )),), true) . '",
		...
		' . $__compilerTemp1 . '
		' . $__compilerTemp2 . '
		"dateCreated": "' . $__templater->filter($__templater->fn('date', array($__vars['resource']['resource_date'], 'c', ), false), array(array('escape', array('json', )),), true) . '",
		"dateModified": "' . $__templater->filter($__templater->fn('date', array($__vars['resource']['last_update'], 'c', ), false), array(array('escape', array('json', )),), true) . '",
		' . $__compilerTemp3 . '
		' . $__compilerTemp4 . '
		"author": {
			"@type": "Person",
...
<div class="block">
	...
	<div class="block-container">
		<div class="block-body lbContainer js-resourceBody"
			...
			data-lb-caption-desc="' . ($__vars['resource']['User'] ? $__templater->escape($__vars['resource']['User']['username']) : $__templater->escape($__vars['resource']['username'])) . ' &middot; ' . $__templater->fn('date_time', array($__vars['resource']['resource_date'], ), true) . '">
	...
	if ($__vars['trimmedDescription']) {
		...
		if (!$__vars['xf']['visitor']['user_id']) {
			$__finalCompiled .= '
								<a href="' . $__templater->fn('link', array('login', ), true) . '" data-xf-click="overlay">' . 'Đăng nhập hoặc đăng ký ngay.' . '</a>
							';
		}
		$__finalCompiled .= '
						</div>
		...
		$__finalCompiled .= '
						' . $__templater->fn('bb_code', array($__vars['description']['message'], 'resource_update', $__vars['description'], ), true) . '
					';
	}
	$__finalCompiled .= '
	...
	if ($__templater->method($__vars['description'], 'canReport', array())) {
		$__compilerTemp10 .= '
											<a href="' . $__templater->fn('link', array('resources/update/report', $__vars['description'], ), true) . '"
	...
	if ($__templater->method($__vars['resource'], 'canEdit', array())) {
		$__compilerTemp10 .= '
											<a href="' . $__templater->fn('link', array('resources/edit', $__vars['resource'], ), true) . '"
	...
	if ($__templater->method($__vars['description'], 'canDelete', array('soft', ))) {
		$__compilerTemp10 .= '
											<a href="' . $__templater->fn('link', array('resources/delete', $__vars['description'], ), true) . '"
	...
	if ($__templater->method($__vars['xf']['visitor'], 'canViewIps', array()) AND $__vars['description']['ip_id']) {
		$__compilerTemp10 .= '
											<a href="' . $__templater->fn('link', array('resources/update/ip', $__vars['description'], ), true) . '"
	...
	if ($__templater->method($__vars['description'], 'canWarn', array())) {
		$__compilerTemp10 .= '
											<a href="' . $__templater->fn('link', array('resources/update/warn', $__vars['description'], ), true) . '"
		...
		$__compilerTemp10 .= '
											<a href="' . $__templater->fn('link', array('warnings', array('warning_id' => $__vars['description']['warning_id'], ), ), true) . '"
	...
	$__finalCompiled .= '
					...
					<div class="reactionsBar js-reactionsList ' . ($__vars['description']['reactions'] ? 'is-active' : '') . '">
						' . $__templater->fn('reactions', array($__vars['description'], 'resources/update/reactions', array())) . '
					</div>
				...
				<div class="resourceBody-sidebar">
					<div class="resourceSidebarGroup">
						<dl class="pairs pairs--justified">
							<dt>' . 'Author' . '</dt>
							<dd>' . $__templater->fn('username_link', array($__vars['resource']['User'], false, array(
	...
	$__finalCompiled .= '
						<dl class="pairs pairs--justified">
							<dt>' . 'Xem' . '</dt>
							<dd>' . $__templater->filter($__templater->fn('max', array($__vars['resource']['view_count'], $__vars['resource']['download_count'], 1, ), false), array(array('number_short', array()),), true) . '</dd>
						</dl>
						<dl class="pairs pairs--justified">
			...
			foreach ($__vars['latestUpdates'] AS $__vars['update']) {
				...
				<li class="block-row block-row--separated">
					<h3 class="block-textHeader">
						<a href="' . $__templater->fn('link', array('resources/update', $__vars['update'], ), true) . '">' . $__templater->escape($__vars['update']['title']) . '</a>
					</h3>
					<div>' . $__templater->fn('snippet', array($__vars['update']['message'], 100, array('stripBbCode' => true, ), ), true) . '</div>
			';
			}
		}
		$__finalCompiled .= '
