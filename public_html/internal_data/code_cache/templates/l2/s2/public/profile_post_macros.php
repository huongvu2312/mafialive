...
'profile_post' => function($__templater, array $__arguments, array $__vars)
{
	...
	$__finalCompiled .= '
						...
						<article class="message-body">
							' . $__templater->fn('bb_code', array($__vars['profilePost']['message'], 'profile_post', $__vars['profilePost'], ), true) . '
						</article>
					</div>
	...
	$__compilerTemp1 .= '
		...
		'list' => '< .message | .js-reactionsList',
									...
									';
	if ($__templater->method($__vars['profilePost'], 'canComment', array()) AND $__templater->fn('property', array('profilePostCommentToggle', ), false)) {
		$__compilerTemp1 .= '
	...
	if (($__vars['profilePost']['message_state'] == 'deleted') AND $__templater->method($__vars['profilePost'], 'canUndelete', array())) {
		$__compilerTemp2 .= '
										<a href="' . $__templater->fn('link', array('profile-posts/undelete', $__vars['profilePost'], ), true) . '" data-xf-click="overlay"
	...
	if ($__vars['profilePost']['reactions']) {
		$__finalCompiled .= '
								' . $__templater->fn('reactions', array($__vars['profilePost'], 'profile-posts/reactions', array())) . '
							';
	}
	$__finalCompiled .= '</div>
	...
	if ($__templater->method($__vars['profilePost'], 'canComment', array())) {
		...
		$__finalCompiled .= '
								<div class="message-responseRow js-commentsTarget-' . $__templater->escape($__vars['profilePost']['profile_post_id']) . ' ' . ($__templater->fn('property', array('profilePostCommentToggle', ), false) ? 'toggleTarget' : '') . '">
									';
...
'profile_post_deleted' => function($__templater, array $__arguments, array $__vars)
{
	...
	<div class="message message--simple' . ($__templater->method($__vars['profilePost'], 'isIgnored', array()) ? ' is-ignored' : '') . ' js-inlineModContainer"
		...
		'log' => $__vars['profilePost']['DeletionLog'],
					...
					<a href="' . $__templater->fn('link', array('profile-posts/show', $__vars['profilePost'], ), true) . '" class="u-jsOnly" data-xf-click="inserter" data-replace="[data-content=profile-post-' . $__templater->escape($__vars['profilePost']['profile_post_id']) . ']">' . 'Hiện' . $__vars['xf']['language']['ellipsis'] . '</a>
		...
		$__finalCompiled .= '
				<div class="contentRow-faderContainer">
					<div class="contentRow-faderContent">
						' . $__templater->fn('bb_code', array($__vars['profilePost']['message'], 'profile_post', $__vars['profilePost'], array('simpleUnfurl' => true, ), ), true) . '
					</div>
...
'comment' => function($__templater, array $__arguments, array $__vars)
{
	...
	$__finalCompiled .= '
		...
		'class' => 'comment-user',
								...
								<article class="comment-body">' . $__templater->fn('bb_code', array($__vars['comment']['message'], 'profile_post_comment', $__vars['comment'], ), true) . '</article>
							</div>
						</div>
	...
	if (($__vars['comment']['message_state'] == 'deleted') AND $__templater->method($__vars['comment'], 'canUndelete', array())) {
		$__finalCompiled .= '
										<a href="' . $__templater->fn('link', array('profile-posts/comments/undelete', $__vars['comment'], ), true) . '" data-xf-click="overlay"
											class="actionBar-action actionBar-action--undelete actionBar-action--menuItem">' . 'Undelete' . '</a>
										';
		$__vars['hasActionBarMenu'] = true;
		$__finalCompiled .= '
	...
	if ($__vars['comment']['reactions']) {
		$__finalCompiled .= '
								' . $__templater->fn('reactions', array($__vars['comment'], 'profile-posts/comments/reactions', array())) . '
							';
	}
	$__finalCompiled .= '</div>
						</footer>
