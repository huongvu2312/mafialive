...
';
	...
	' . $__templater->button('Add API key', array(
		'href' => $__templater->fn('link', array('api-keys/add', ), false),
		'icon' => 'add',
	), '', array(
	)) . '
');
...
';
		...
		if ($__templater->isTraversable($__vars['apiKeys'])) {
			foreach ($__vars['apiKeys'] AS $__vars['apiKey']) {
				...
				$__compilerTemp1 .= $__templater->dataRow(array(
					...
					'href' => $__templater->fn('link', array('api-keys/edit', $__vars['apiKey'], ), false),
					'delete' => $__templater->fn('link', array('api-keys/delete', $__vars['apiKey'], ), false),
					'explain' => '
	...
	', array(
			'action' => $__templater->fn('link', array('api-keys/toggle', ), false),
			'class' => 'block',
			'ajax' => 'true',
		)) . '
';
