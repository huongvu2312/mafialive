<?php
// FROM HASH: fcc2666152da509ffb3adc3072b90ce1
...
';
	if ($__templater->method($__vars['user'], 'isUpdate', array())) {
		...
		if (!$__vars['user']['is_super_admin']) {
			$__compilerTemp4 .= '
					<a href="' . $__templater->fn('link', array('users/remove-reactions', $__vars['user'], ), true) . '" class="menu-linkRow" data-xf-click="overlay">' . 'Remove reactions' . '</a>
...
<div class="block">
	...
	';
		...
		if ($__vars['user']['is_admin']) {
			$__compilerTemp6 .= '
					<li><a href="' . $__templater->fn('link', array('admins/edit', $__vars['user'], ), true) . '">' . ($__vars['user']['is_super_admin'] ? 'Super Administrator' : 'Quản trị viên') . '</a></li>
				';
		}
		$__compilerTemp6 .= '
				';
