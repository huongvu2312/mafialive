...
';
	...
	' . $__templater->button('', array(
			'href' => $__templater->fn('link', array('api-keys/delete', $__vars['apiKey'], ), false),
			'icon' => 'delete',
			'overlay' => 'true',
		), '', array(
		)) . '
...
';
	...
	if ($__templater->method($__vars['apiKey'], 'isUpdate', array())) {
		$__compilerTemp1 .= '
				' . $__templater->formRow('
					...
					', array(
			'href' => $__templater->fn('link', array('api-keys/view-key', $__vars['apiKey'], ), false),
			...
			'class' => 'button--link',
					...
					', array(
			'href' => $__templater->fn('link', array('api-keys/regenerate', $__vars['apiKey'], ), false),
			'data-xf-click' => 'overlay',
			'class' => 'button--link',
		), '', array(
		)) . '
	...
	if ($__templater->method($__vars['apiKey'], 'isUpdate', array())) {
		$__compilerTemp2 .= '
			...
			'apiKey' => $__vars['apiKey'],
				...
				' . $__templater->formRow('
					' . $__templater->fn('date_dynamic', array($__vars['apiKey']['creation_date'], array(
		))) . '
					&middot;
		...
		if ($__vars['apiKey']['last_use_date']) {
			$__compilerTemp3 .= '
						' . $__templater->fn('date_dynamic', array($__vars['apiKey']['last_use_date'], array(
			))) . '
					';
		} else {
			$__compilerTemp3 .= '
	...
	<div class="block-container">
		...
		'name' => 'title',
		'maxlength' => $__templater->fn('max_length', array($__vars['apiKey'], 'title', ), false),
...
', array(
		'action' => $__templater->fn('link', array('api-keys/save', $__vars['apiKey'], ), false),
		'ajax' => 'true',
