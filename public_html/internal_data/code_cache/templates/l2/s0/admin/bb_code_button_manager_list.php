...
';
	...
	' . $__templater->button('Add dropdown', array(
		'href' => $__templater->fn('link', array('button-manager/dropdown/add', ), false),
		'icon' => 'add',
	), '', array(
	)) . '
');
	...
	if ($__templater->isTraversable($__vars['toolbarTypes'])) {
		foreach ($__vars['toolbarTypes'] AS $__vars['type'] => $__vars['phrases']) {
			$__compilerTemp1 .= '
				...
				'href' => $__templater->fn('link', array('button-manager/edit', null, array('type' => $__vars['type'], ), ), false),
			), array()) . '
				';
		}
	}
...
';
		...
		if ($__templater->isTraversable($__vars['editorDropdowns'])) {
			foreach ($__vars['editorDropdowns'] AS $__vars['editorDropdown']) {
				$__compilerTemp2 .= '
					...
					'href' => $__templater->fn('link', array('button-manager/dropdown/edit', $__vars['editorDropdown'], ), false),
					'icon' => ($__vars['editorDropdown']['icon'] ?: 'none'),
					'delete' => $__templater->fn('link', array('button-manager/dropdown/delete', $__vars['editorDropdown'], ), false),
				), array(array(
	...
	', array(
			'action' => $__templater->fn('link', array('button-manager/dropdown/toggle', ), false),
			'class' => 'block',
			'ajax' => 'true',
		)) . '
';
