			...
			<li class="block-row block-row--separated">
				<div class="contentRow">
					<span class="contentRow-figure">
						' . $__templater->fn('avatar', array($__vars['reaction']['ReactionUser'], 's', false, array(
			))) . '
					</span>
	...
	if ($__vars['hasNext']) {
		$__finalCompiled .= '
			<li class="block-footer">
				...
				', array(
			'href' => $__templater->fn('link', array('account/reactions', null, array('reaction_id' => $__vars['reactionId'], 'page' => $__vars['page'] + 1, ), ), false),
...
';
	if ($__templater->fn('count', array($__vars['reactions'], ), false) > 0) {
		$__finalCompiled .= '
	';
		if (!$__vars['listOnly']) {
			$__finalCompiled .= '
			...
			if ($__templater->isTraversable($__vars['tabSummary'])) {
				foreach ($__vars['tabSummary'] AS $__vars['reactionId'] => $__vars['total']) {
					...
					if ($__vars['reactionId'] == $__vars['activeReactionId']) {
						...
						$__finalCompiled .= '
							<li data-href="' . $__templater->fn('link', array('account/reactions', null, array('reaction_id' => $__vars['reactionId'], 'list_only' => 1, ), ), true) . '" class="' . (($__vars['reactionId'] == $__vars['activeReactionId']) ? 'is-active' : '') . '" role="tabpanel" id="reaction-' . $__templater->escape($__vars['reactionId']) . '">
