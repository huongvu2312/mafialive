		...
		<div class="contentRow">
			<div class="contentRow-main">
				<h4 class="contentRow-title">
					<a href="' . $__templater->fn('link', array('media', $__vars['mediaItem'], ), true) . '" class="fauxBlockLink-blockLink u-cloaked">' . $__templater->escape($__vars['mediaItem']['title']) . '</a>
				</h4>
	...
	if ($__vars['xf']['options']['enableTagging'] AND $__vars['mediaItem']['tags']) {
		...
		if ($__templater->isTraversable($__vars['mediaItem']['tags'])) {
			foreach ($__vars['mediaItem']['tags'] AS $__vars['tag']) {
				$__finalCompiled .= '
									<a href="' . $__templater->fn('link', array('tags', $__vars['tag'], ), true) . '" class="tagItem">' . $__templater->escape($__vars['tag']['tag']) . '</a>
								';
			}
		}
		$__finalCompiled .= '
	...
	if ($__vars['mediaItem']['description']) {
		$__finalCompiled .= '
					<div class="contentRow-snippet">
						' . $__templater->fn('structured_text', array($__templater->fn('snippet', array($__vars['mediaItem']['description'], 100, ), false), ), true) . '
					</div>
				';
	}
	$__finalCompiled .= '
