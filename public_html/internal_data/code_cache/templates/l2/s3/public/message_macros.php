<?php
// FROM HASH: 3907a7fb5a9e7d8f3556c6531871195c
return array('macros' => array('user_info' => function($__templater, array $__arguments, array $__vars)
{
	...
	$__finalCompiled .= '
		...
		'itemprop' => 'jobTitle',
				...
				';
	if ($__vars['xf']['options']['showMessageOnlineStatus'] AND ($__vars['user'] AND ($__templater->method($__vars['user'], 'isOnline', array()) AND ($__templater->fn('property', array('uix_onlineIndicator', ), false) == 'userbanner')))) {
		$__finalCompiled .= '
	...
	$__finalCompiled .= '
				';
	if (($__vars['user']['user_id'] == $__vars['threadUserId']) AND ($__templater->fn('property', array('uix_originalPoster', ), false) != 'none')) {
		$__finalCompiled .= '
					';
		if ($__templater->fn('property', array('uix_originalPoster', ), false) == 'userbanner') {
			$__finalCompiled .= '
					...
					';
		} else if ($__templater->fn('property', array('uix_originalPoster', ), false) == 'icon') {
			$__finalCompiled .= '
	...
	if ($__vars['user']['user_id']) {
		...
		if ($__vars['extras']['location'] AND $__vars['user']['Profile']['location']) {
			...
			if ($__templater->fn('property', array('uix_postBitIcons', ), false)) {
				$__compilerTemp1 .= '
													<span data-xf-init="tooltip" title="' . 'Địa chỉ' . '">
