<?php
// FROM HASH: 628594f79a167cd399e0ff44ed03fdb2
return array('macros' => array('link' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__templater->method($__vars['content'], 'canBookmark', array())) {
		...
		<a href="' . $__templater->escape($__vars['confirmUrl']) . '" class="bookmarkLink ' . $__templater->escape($__vars['class']) . ' ' . ($__templater->method($__vars['content'], 'isBookmarked', array()) ? 'is-bookmarked' : '') . '"
			...
			data-sk-bookmarked="addClass:is-bookmarked, ' . $__templater->escape($__vars['editText']) . '"
			data-sk-bookmarkremoved="removeClass:is-bookmarked, ' . $__templater->escape($__vars['addText']) . '">';
		$__compilerTemp1 = '';
...
'row' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__vars['bookmark']['has_custom_icon']) {
		...
		';
	} else if ($__vars['content']['User'] OR $__vars['content']['username']) {
		$__finalCompiled .= '
			<div class="contentRow-figure contentRow-figure--fixedBookmarkIcon">
				' . $__templater->fn('avatar', array($__vars['content']['User'], 's', false, array(
			'href' => '',
	...
	if ($__vars['bookmark']['message']) {
		$__finalCompiled .= '
					' . $__templater->fn('structured_text', array($__vars['bookmark']['message'], ), true) . '
				';
	} else {
		$__finalCompiled .= '
					';
	...
	if ($__vars['bookmark']['content_user'] OR $__templater->method($__vars['content'], 'isValidKey', array('username', ))) {
		$__finalCompiled .= '
			...
			'title' => 'User',
					...
					' . $__templater->fn('username_link', array($__vars['bookmark']['content_user'], false, array(
	...
	if ($__vars['bookmark']['labels']) {
		...
		if ($__templater->isTraversable($__vars['bookmark']['labels'])) {
			foreach ($__vars['bookmark']['labels'] AS $__vars['label']) {
				$__finalCompiled .= '
						<a href="' . $__templater->fn('link', array('account/bookmarks', null, array('label' => $__vars['label']['label'], ), ), true) . '" class="tagItem" dir="auto">' . $__templater->escape($__vars['label']['label']) . '</a>
					';
			}
		}
		$__finalCompiled .= '
