	...
	if ($__templater->isTraversable($__vars['recent'])) {
		foreach ($__vars['recent'] AS $__vars['shortname'] => $__vars['emoji']) {
			...
			if ($__vars['emoji']['smilie_id']) {
				$__finalCompiled .= '
						' . $__templater->fn('smilie', array($__vars['shortname'], ), true) . '
					';
			} else {
				$__finalCompiled .= '
		...
		';
			...
			if ($__templater->isTraversable($__vars['groupedSmilies'][$__vars['categoryId']])) {
				foreach ($__vars['groupedSmilies'][$__vars['categoryId']] AS $__vars['smilie']) {
					$__compilerTemp1 .= '
							<li><a class="js-emoji" data-shortname="' . $__templater->escape($__vars['smilie']['smilie_text_options']['0']) . '">' . $__templater->fn('smilie', array($__vars['smilie']['smilie_text_options']['0'], ), true) . '</a></li>
						';
				}
			}
			$__compilerTemp1 .= '
