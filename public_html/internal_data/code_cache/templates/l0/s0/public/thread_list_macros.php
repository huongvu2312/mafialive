<?php
// FROM HASH: aed5b49c83721c6eb3199bec78743ae6
return array('macros' => array('item' => function($__templater, array $__arguments, array $__vars)
{
	...
	$__compilerTemp1 .= '
					';
	if (($__templater->fn('property', array('reactionSummaryOnLists', ), false) == 'status') AND $__vars['thread']['first_post_reactions']) {
		$__compilerTemp1 .= '
						<li>' . $__templater->fn('reactions_summary', array($__vars['thread']['first_post_reactions'])) . '</li>
					';
	}
	$__compilerTemp1 .= '
					';
	...
	$__compilerTemp2 .= '
						';
	if (($__templater->fn('property', array('reactionSummaryOnLists', ), false) == 'minor_opposite') AND $__vars['thread']['first_post_reactions']) {
		$__compilerTemp2 .= '
							<li>' . $__templater->fn('reactions_summary', array($__vars['thread']['first_post_reactions'])) . '</li>
						';
	}
	...
	$__finalCompiled .= '
		...
		<div class="structItem-cell structItem-cell--meta" title="' . $__templater->filter('First message reaction score' . $__vars['xf']['language']['label_separator'], array(array('for_attr', array()),), true) . ' ' . $__templater->filter($__vars['thread']['first_post_reaction_score'], array(array('number', array()),), true) . '">
			...
			<dl class="pairs pairs--justified structItem-minor">
				<dt>' . 'Views' . '</dt>
				<dd>' . (($__vars['thread']['discussion_type'] == 'redirect') ? '&ndash;' : (($__vars['thread']['view_count'] > $__vars['thread']['reply_count']) ? $__templater->filter($__vars['thread']['view_count'], array(array('number_short', array()),), true) : $__templater->fn('number_short', array($__vars['thread']['reply_count'] + 1, ), true))) . '</dd>
			</dl>
		</div>
	...
	if ($__vars['thread']['discussion_type'] == 'redirect') {
		...
		if ($__templater->method($__vars['xf']['visitor'], 'isIgnoring', array($__vars['thread']['last_post_user_id'], ))) {
			...
			$__finalCompiled .= '
						' . $__templater->fn('username_link', array($__vars['thread']['LastPoster'], false, array(
