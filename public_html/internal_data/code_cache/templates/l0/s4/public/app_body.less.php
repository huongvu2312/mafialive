<?php
// FROM HASH: 6d4a8ce76025c8a0424585f24ed4a08a
...
.p-body-inner
{
	...
	$__finalCompiled .= '
	transition: @uix_moveOut max-width .3s, @uix_moveOut left .3s, @uix_moveOut width .3s;
	display: flex;
...
.p-body-content
{
	...
	if (($__templater->fn('property', array('uix_sidebarLocation', ), false) == 'right')) {
		$__finalCompiled .= '
			transition: @uix_moveOut width .25s, @uix_moveOut max-width .25s;

			.uix_sidebarCollapsed & {
				transition: @uix_moveIn width .25s .25s, @uix_moveIn max-width .25s .25s;
			}
		';
	} else {
		$__finalCompiled .= '
